...
function removePipeAction:isValid()
	local object = self.square and WaterSupplyPipe.findAnyPipeObject(self.square)
	return object ~= nil and not WaterSupplyPipe.hasStoredItems(object)
end
...
function removePipeAction:perform()
	if isClient() then
		...
		sendClientCommand(self.character, 'WaterPipe', 'pickUp', args)
	else
